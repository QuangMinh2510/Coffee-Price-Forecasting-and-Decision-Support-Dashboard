...
from sklearn.ensemble import ExtraTreesRegressor, GradientBoostingRegressor, HistGradientBoostingRegressor, RandomForestRegressor, StackingRegressor
...
def mape(actual, predicted):
    ...
    return float(np.mean(np.abs((actual[mask] - predicted[mask]) / actual[mask])) * 100) if mask.any() else np.nan
...
def model_factories():
    ...
    tree = lambda model: Pipeline([
        ("imputer", SimpleImputer(strategy="median")),
        ("model", model),
    ])
    factories = {
        "LinearRegression": lambda: scaled(LinearRegression()),
        "Ridge": lambda: scaled(Ridge(alpha=10.0)),
        "ElasticNet": lambda: scaled(ElasticNet(alpha=0.001, l1_ratio=0.3, max_iter=10000)),
        "SVR": lambda: scaled(SVR(C=30.0, epsilon=0.1, gamma="scale")),
        "kNN": lambda: scaled(KNeighborsRegressor(n_neighbors=10, weights="distance")),
        "RandomForest": lambda: tree(RandomForestRegressor(n_estimators=350, min_samples_leaf=2, random_state=42, n_jobs=-1)),
        "ExtraTrees": lambda: tree(ExtraTreesRegressor(n_estimators=350, min_samples_leaf=2, random_state=42, n_jobs=-1)),
        "GradientBoosting": lambda: tree(GradientBoostingRegressor(n_estimators=250, learning_rate=0.035, max_depth=3, random_state=42)),
        "HistGradientBoosting": lambda: tree(HistGradientBoostingRegressor(max_iter=300, learning_rate=0.04, max_leaf_nodes=31, random_state=42)),
    }
    if HAS_XGBOOST:
        factories["XGBoost"] = lambda: tree(XGBRegressor(
            n_estimators=500, learning_rate=0.025, max_depth=5, subsample=0.85,
            colsample_bytree=0.85, objective="reg:squarederror", random_state=42, n_jobs=-1,
        ))
    if HAS_LIGHTGBM:
        factories["LightGBM"] = lambda: tree(LGBMRegressor(
...
def build_dataset(path: Path, horizon: int):
    ...
    frame = frame.dropna(subset=[date_col, target_col]).sort_values(date_col).drop_duplicates(date_col, keep="last")

    numeric = frame.select_dtypes(include="number").columns.tolist()
    forbidden = {target_col}
    forbidden.update(column for column in numeric if column.lower().startswith(("target", "future", "lead")))
    features = [column for column in numeric if column not in forbidden]

    # Tạo các đặc trưng an toàn từ giá tại thời điểm hiện tại và quá khứ.
...
def main():
    ...
    selected = args.models or list(factories)
    unknown = sorted(set(selected) - set(factories))
    if unknown:
        raise ValueError(f"Model không khả dụng: {unknown}. Có thể dùng: {sorted(factories)}")
    ...
    for horizon in args.horizons:
        ...
        for model_name in selected:
            ...
            metrics, predictions, bundle, importance = train_one(
                dataset, date_col, target_col, features, horizon, model_name, factories[model_name]
            )
            registry_rows.append(metrics)
            safe_name = model_name.lower().replace(" ", "_")
            joblib.dump(bundle, MODEL_DIR / f"model_{safe_name}_h{horizon}.joblib")
            predictions.to_csv(PREDICTION_DIR / f"prediction_{safe_name}_h{horizon}.csv", index=False, encoding="utf-8-sig")
            if importance is not None:
                importance.to_csv(IMPORTANCE_DIR / f"importance_{safe_name}_h{horizon}.csv", index=False, encoding="utf-8-sig")

    registry = pd.DataFrame(registry_rows).sort_values(["horizon", "RMSE"])
    registry.to_csv(MODEL_DIR / "model_registry.csv", index=False, encoding="utf-8-sig")
    print(registry.to_string(index=False))
    print("\nKhông tạo ảnh PNG. Dashboard tự vẽ biểu đồ từ CSV trong results/.")
